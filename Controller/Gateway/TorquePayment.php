...
namespace TorqueIntegrations\Custompayment\Controller\Gateway;

use Magento\Framework\App\Action\Context;

// Torque_Library is just the contents of library.php. It's super messy but works for now
...
class Torque
{
    ...
    {
        $this->torque_daemon = new Torque_Library('http://' . $rpc_address . ':'. $rpc_port . '/json_rpc'); // TODO: Get address:port from admin panel
    }

    public function retriveprice($currency)
    {
        $xtc_price = file_get_contents('https://api.coinmarketcap.com/v1/ticker/torque/');
    ...
    public function changeto($amount, $currency)
    {
        ...
        $converted_rounded = round($price_converted, 11); //the Torque wallet can't handle decimals smaller than 0.000000000001
    ...
    {
        $integrated_address = $this->torque_daemon->make_integrated_address($payment_id);
        $parsed_address = $integrated_address['integrated_address'];
        return $parsed_address;
    }
}

class TorquePayment extends \Magento\Framework\App\Action\Action
{
    public function __construct(\TorqueIntegrations\Custompayment\Helper\Data $helper,\Magento\Checkout\Model\Session $checkoutSession, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Checkout\Model\Cart $cart, \Magento\Framework\App\Action\Context $context)
    {
        $this->helper = $helper;
    ...
    public function execute()
    {
        ...
        $torque = new Torque($rpc_address, $rpc_port);

        $currency = 'USD';
        $grandTotal = $this->checkoutSession->getQuote()->getGrandTotal();

        $price = $torque->changeto($grandTotal, $currency);
        $payment_id = $torque->paymentid_cookie();
        $integrated_address = $torque->integrated_address($payment_id);
        $status = $torque->verify_payment($payment_id, $price);
        if($status)
        {
        ...
        <!--Let browser know website is optimized for mobile-->
            ...
            <span class='xtc-payment-text-header'><h2>TORQUE PAYMENT $status_message</h2></span>
            </div>
            <!-- end header -->
            ...
            <div class='xtc-amount-box'>$price</div><div class='xtc-box'>XTC</div>
            </div>

            <div class='xtc-address'>
            <span class='xtc-label'>To this address:</span>
            <div class='xtc-address-box'>$integrated_address</div>
            </div>
            <div class='xtc-qr-code'>
            <span class='xtc-label'>Or scan QR:</span>
            <div class='xtc-qr-code-box'><img src='https://api.qrserver.com/v1/create-qr-code/? size=200x200&data=torque:$integrated_address' /></div>
            </div>
            ...
            <a href='#'>Help</a> | <a href='#'>About Torque</a>
            </div>
            ...
            <div class='row'>
            <div class='col-lg-2'>
                    <form id='first' action='TorquePayment' action='post'>
                        Firstname
                        <input type='text' name='first' class='form-control' value=$first>
                    </form>
            </div>
            <div class='col-lg-3'>
            <form id='last' action='TorquePayment' action='post'>
                Lastname
            <input type='text' name='last' class='form-control' value=$last>
            </form>
            </div>
            <div class='col-xs-4'>
            <form id='email' action='TorquePayment' action='post'>
                E-mail
            <input type='text' name='email' value=$email>
            </form>
            </div>
            <form id='phone' action='TorquePayment' action='post'>
                Phone Number
            ...
            <br></br>
            <form id='city' action='TorquePayment' action='post'>
                City
            <input type='text' name='city' value=$city>
            </form>
            <form id='street' action='TorquePayment' action='post'>
                Street
            <input type='text' name='street' value=$street>
            </form>
            <form id='postal' action='TorquePayment' action='post'>
                Postal Code
        ...
        if(isset($_GET['ordered']))
        {
            ...
            {
                echo "<script type='text/javascript'>window.location.replace('TorquePayment?first=$first&last=$last&email=$email&phone=$phone&city=$city&street=$street&postal=$postal&country=$country&region=$region&ordered=1')</script>";
            }
        }
    }
}
